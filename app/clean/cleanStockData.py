...
def cleanStockData(file_name : str, start_date_str: str, end_date_str: str) -> List[pd.DataFrame]:
    ...
    for col_name in stock_data.columns.values:
        if col_name == "Date" or col_name ==  "date":
            ...
            stock_data = stock_data.rename(columns={col_name:"Values"})
    ...
    if values_index == 1:
        stock_data = stock_data.rename(columns={stock_data.columns[1]:"Values"})
    
    #Strip Unecessary Data
    
    stock_data =  stock_data.filter(["Date","Values"])
    ...
    #Normalize data:
    std_norm_values = (stock_data["Values"]-stock_data["Values"].mean())/stock_data["Values"].std()    
    min_max_norm_values = (stock_data["Values"]-stock_data["Values"].min())/(stock_data["Values"].max()-stock_data["Values"].min())
